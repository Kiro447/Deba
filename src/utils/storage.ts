...
export async function getVehicles(): Promise<Car[]> {
  if (USE_BACKEND) {
    try { return await api.get<Car[]>('/api/vehicles') } catch { return [] }
  }
...
export async function createReservation(data: {
  ...
  if (USE_BACKEND) { await api.post('/api/reservations', data); return }
...
export async function getReservations(): Promise<Reservation[]> {
  if (USE_BACKEND) {
    try { return await api.get<Reservation[]>('/api/reservations') } catch { return [] }
  }
...
export async function updateReservationStatus(
  ...
  if (USE_BACKEND) { await api.put(`/api/reservations/${id}/status`, { status }); return }
